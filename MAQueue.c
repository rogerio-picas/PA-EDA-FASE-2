/*
Grafos: Travessia em Largura com Matriz de Adjacências : BFT
 ...
 *
 *      @param [in]     adjMatrix   
 *      @param [in,out] setVertices (conjunto de vertices)
 *      @param [in]     v			(vertice inicial - ponto de partida)
 */
void breadthFirstSearch(int adjMatrix[][MAX], Vertice *setVertices[], int v) {
   int i; 
   ...
   setVertices[v]->visitado = true; 
   //processa vertice: mostra e calcula distância
   ProcessaVertice(setVertices,v);
   //insere vertice na queue
   insertQueue(queue,&rear, v, &queueItemCount); 
  
   while(!isQueueEmpty(queueItemCount)) {
	   //encontra vertice não visitado que está à frente da queue
      int tempVertex = nextQueue(queue, &queueItemCount,&front);
 
      //para cada adjacente não visitado
      while((uv = GetVertAdjunNaoVisitado(adjMatrix, setVertices,tempVertex)) != -1)
	  {
		//marca
         setVertices[uv]->visitado = true; 
		 //processa
         ProcessaVertice(setVertices,uv);
		 //insere na queue
         insertQueue(queue, &rear, uv,&queueItemCount);
      } 
